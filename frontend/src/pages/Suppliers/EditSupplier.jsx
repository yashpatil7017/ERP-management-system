...
import { getSuppliers, updateSupplier } from '../../services/supplierService';
  ...
  useEffect(() => {
    ...
    async function loadSupplier() {
      ...
      try {
        ...
        if (supplierFromState?._id === id) {
          setFormData({
            name: supplierFromState.name || '',
            email: supplierFromState.email || '',
            phone: supplierFromState.phone || '',
            address: supplierFromState.address || '',
            city: supplierFromState.city || '',
            taxNumber: supplierFromState.taxNumber || '',
            isActive: Boolean(supplierFromState.isActive),
          });
          return;
        }

        // Backend does not provide a dedicated get-by-id supplier endpoint.
        const data = await getSuppliers(1, 1000, '');
        const found = (data?.suppliers || []).find((item) => item._id === id);
  ...
  if (loadError) {
    return (
      <Box sx={{ py: 6 }}>
        ...
        <Button sx={{ mt: 2 }} component={Link} to="/suppliers" variant="outlined">
          Back to Suppliers
        </Button>
      </Box>
    );
